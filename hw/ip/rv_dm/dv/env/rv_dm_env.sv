...
class rv_dm_env extends cip_base_env #(
  ...
  function void connect_phase(uvm_phase phase);
    ...
    if (cfg.en_scb) begin
      if (cfg.m_jtag_dtm_ral != null) begin
        m_jtag_agent.monitor.analysis_port.connect(
            m_jtag_dmi_monitor.jtag_item_fifo.analysis_export);
        m_jtag_dmi_monitor.non_dmi_jtag_dtm_analysis_port.connect(
            scoreboard.jtag_non_dmi_dtm_fifo.analysis_export);
      end

      if (cfg.jtag_dmi_ral != null) begin
        m_jtag_dmi_monitor.analysis_port.connect(
            m_sba_access_monitor.jtag_dmi_fifo.analysis_export);
