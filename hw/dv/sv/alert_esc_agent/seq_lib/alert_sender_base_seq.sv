...
task alert_sender_base_seq::body();
  ...
  `uvm_info(get_full_name(),
            $sformatf("seq_item: send_alert=%0b, ping_rsp=%0b, m_int_err_cyc=%0b",
                      req.s_alert_send, req.s_alert_ping_rsp, req.m_int_err_cyc),
            UVM_MEDIUM)

  finish_item(req);
  get_response(rsp);

  `uvm_info(get_full_name(), "alert sender transfer done", UVM_HIGH)
endtask : body
