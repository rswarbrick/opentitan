...
task alert_sender_driver::send_item(bit is_ping_rsp, alert_seq_item item);
  ...
  `uvm_info("driving_item",
            $sformatf("starting sender item, alert_send=%0b, ping_rsp=%0b, m_int_err_cyc=%0b",
                      item.s_alert_send, item.s_alert_ping_rsp, item.m_int_err_cyc),
            UVM_HIGH)
  fork : isolation_fork begin
    fork
      wait (cfg.in_reset);
  ...
  `uvm_info("driving_item",
            $sformatf("finished sender item, alert_send=%0b, ping_rsp=%0b, m_int_err_cyc=%0b",
                      item.s_alert_send, item.s_alert_ping_rsp, item.m_int_err_cyc),
            UVM_HIGH)
