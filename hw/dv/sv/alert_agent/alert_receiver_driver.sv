...
// A driver that behaves as an alert_receiver. This is used in an alert_agent when configured in
// "device mode"
  ...
  fork : isolation_fork begin
    fork
      ...
      forever begin
        ...
        case (item.m_trans_type)
          AlertSigTrans:  m_pending_alert_rsps.write(item);
          AlertPingTrans: m_pending_pings.write(item);
          default: `uvm_fatal(get_full_name(), "Unknown transaction type")
        endcase
      end
    join_any
    disable fork;
...
task alert_receiver_driver::send_ping(alert_seq_item item);
  ...
  if (item.m_int_err_cyc) begin
    `uvm_fatal(get_full_name(), "Cannot send ping with m_int_err_cyc > 0: not supported by driver.")
  end

  // Wait for item.m_ping_delay cycles (which might be quite a while) before sending the ping. There
  // may be other alerts happening in the meantime, but this task doesn't need to worry about them.
  // Exit early on reset
  fork : isolation_fork begin
    fork
      wait(cfg.in_reset);
      begin
        repeat (item.m_ping_delay) @(cfg.vif.receiver_cb);
        set_ping();
      end
    join_any
    disable fork;
