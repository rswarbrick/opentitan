...
class alert_sender_driver extends alert_base_driver;
  ...
  //
  // An implementation of alert_base_driver::drive_req
...
task alert_sender_driver::drive_reqs_with_lpg_mode(bit en_alert_lpg);
  forever begin
    ...
    case (item.m_trans_type)
      AlertSigTrans:  send_item(1'b0, rsp);
      AlertPingTrans: send_item(1'b1, rsp);
...
task alert_sender_driver::send_item(bit is_ping_rsp, alert_seq_item item);
  ...
  `uvm_info("driving_item",
            $sformatf("starting sender item: %0s, m_int_err_cyc=%0b",
                      item.m_trans_type.name(), item.m_int_err_cyc),
            UVM_HIGH)
  fork : isolation_fork begin
    fork
      wait (cfg.in_reset);
